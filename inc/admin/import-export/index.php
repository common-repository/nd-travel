...
function nd_travel_settings_menu_import_export() {
  ...
  //ajax results
  $nd_travel_import_settings_params = array(
      'nd_travel_ajaxurl_import_settings' => admin_url('admin-ajax.php'),
      'nd_travel_ajaxnonce_import_settings' => wp_create_nonce('nd_travel_import_settings_nonce'),
  );

  wp_enqueue_script( 'nd_travel_import_sett', esc_url( plugins_url( 'js/nd_travel_import_settings.js', __FILE__ ) ), array( 'jquery' ) ); 
  wp_localize_script( 'nd_travel_import_sett', 'nd_travel_my_vars_import_settings', $nd_travel_import_settings_params ); 
...
function nd_travel_import_settings_php_function() {
  ...
  check_ajax_referer( 'nd_travel_import_settings_nonce', 'nd_travel_import_settings_security' );

  //recover datas
  $nd_travel_value_import_settings = sanitize_text_field($_GET['nd_travel_value_import_settings']);
    ...
    if ( $nd_travel_value_import_settings != '' ) {
      ...
      foreach ($nd_travel_array_options as $nd_travel_array_option) {
        ...
        if ( $nd_travel_new_value != '' ){
          ...
          if ( strpos($nd_travel_option, 'nd_travel_') !== false ) {
            ...
            <div class="notice notice-error is-dismissible nd_travel_margin_0">
              <p>'.__('You do not have permission to change this option','nd-travel').'</p>
            </div>';
  ...
  }else
  {
    ...
    $nd_travel_import_settings_result .= '
      <div class="notice notice-error is-dismissible nd_travel_margin_0">
        <p>'.__('You do not have the privileges to do this.','nd-travel').'</p>
      </div>
    ';
